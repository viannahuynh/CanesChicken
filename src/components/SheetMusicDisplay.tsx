...
export default function SheetMusicDisplay({ music }: SheetMusicDisplayProps) {
  ...
  return (
    <Card className="border-2 border-black/10 shadow-xl bg-white/90 backdrop-blur-sm">
      <CardHeader>
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <CardDescription className="text-gray-600">Your performance visualized as musical notation</CardDescription>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Badge variant="secondary" className="gap-1 bg-black text-white border-black hover:bg-red-600 transition-colors">
              <Key className="w-3 h-3" />
              {music.keySignature}
            </Badge>
            <Badge variant="secondary" className="gap-1 bg-white border-2 border-black text-black hover:bg-black hover:text-white transition-colors">
              <Clock className="w-3 h-3" />
              {music.tempo} BPM
            </Badge>
            <Badge variant="secondary" className="bg-white border-2 border-black text-black hover:bg-black hover:text-white transition-colors">
              {music.timeSignature}
            </Badge>
          </div>
        </div>
      ...
      <CardContent>
        {/* Staff Lines */}
        <div className="bg-white rounded-lg p-8 border-2 border-black/20 shadow-inner relative overflow-hidden">
          <div className="absolute top-2 right-2 text-red-600/20 text-4xl">♫</div>
          <div className="relative">
            {/* Treble Clef Symbol */}
            <div className="absolute left-0 top-1/2 -translate-y-1/2 text-6xl text-black">
              𝄞
            </div>
            ...
            <div className="ml-16 relative" style={{ height: '200px' }}>
              {[0, 1, 2, 3, 4].map((line) => (
                <div
                  key={line}
                  className="absolute w-full border-t-2 border-black/40"
              ...
              <div className="absolute inset-0 flex items-center gap-4 pl-8">
                {music.notes.map((noteData, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className="text-5xl text-black leading-none transition-all group-hover:text-red-600 group-hover:scale-110">
                        {symbol}
                      </div>
          ...
          <div className="mt-8 pt-6 border-t-2 border-black/10 flex gap-6 justify-center text-sm text-gray-700 flex-wrap">
            <div className="flex items-center gap-2 hover:text-red-600 transition-colors cursor-default">
